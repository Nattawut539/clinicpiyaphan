...
ซอฟต์แวร์ผ่าน unit tests 46 ข้อ และได้ทดสอบสร้างฐานข้อมูลใหม่ รัน migrations และเข้าสู่ระบบด้วยบัญชี demo ทั้งสองแล้ว ส่วนการเชื่อมต่อเซนเซอร์ การ calibration และการพิมพ์กับอุปกรณ์จริงยังต้องทดสอบร่วมกับทีมฮาร์ดแวร์ก่อนรับรองการใช้งานจริง
...
`schema.sql` ใช้กับฐานข้อมูลว่าง ไม่ใช่สคริปต์รันซ้ำ หากคำสั่งล้มเหลวให้แก้ก่อนขั้นต่อไป

ไฟล์ใน `database/` แยกหน้าที่ดังนี้:
...
## แชทผู้ใช้กับคลินิก (วัตถุประสงค์ 3.5)

ผู้ใช้ที่เข้าสู่ระบบและยินยอมข้อมูลแล้วเปิดปุ่มแชทล่างขวาได้ทุกหน้าใน `/users` ส่วน `admin`, `doctor`, `super_admin`/`superadmin` มีรายการบทสนทนาร่วมกันทุกหน้าใน `/admin` แสดงชื่อ–นามสกุลผู้ส่งและค้นหาชื่อได้ บัญชี assistant ไม่มีสิทธิ์ใช้แชทนี้

- ข้อความและชื่อผู้ส่งบันทึกใน PostgreSQL; ฝั่งผู้ใช้เข้าถึงได้เฉพาะบทสนทนาตนเอง
- แชทดึงข้อความใหม่ทุกประมาณ 3 วินาทีเมื่อเปิด และอัปเดตรายการ/จำนวนยังไม่อ่านทุกประมาณ 5 วินาทีเมื่อแท็บมองเห็น ไม่แสดงสถานะออนไลน์ที่ระบบยังไม่ได้ตรวจจริง
- ย่อ ขยาย ซ่อน และเปิดกลับได้ ข้อความร่างคงอยู่เมื่อเปลี่ยนหน้าผ่านเมนูเว็บและสลับบทสนทนา; หลังรีเฟรชหน้าคงเฉพาะสถานะหน้าต่าง ข้อความที่ยังไม่ได้ส่งไม่บันทึกลง browser storage
- Enter ส่ง, Shift+Enter ขึ้นบรรทัดใหม่; ข้อความยาวสูงสุด 4,000 ตัวอักษร รองรับประวัติย้อนหลังและข้อความหลายบรรทัด ไม่รองรับไฟล์แนบในรุ่นนี้
- หากส่งไม่สำเร็จ ข้อความยังอยู่ในช่องพิมพ์เพื่อส่งซ้ำ ระบบใช้รหัสข้อความเดิมป้องกันการบันทึกซ้ำ
- จำนวนยังไม่อ่านของเจ้าหน้าที่นับแยกต่อบัญชี ผู้ใช้เห็น “อ่านแล้ว” เมื่อเจ้าหน้าที่คนใดคนหนึ่งอ่านถึงข้อความนั้น

ฐานข้อมูลเดิมต้องอัปเกรดก่อน Deploy ตาม [ขั้นตอนแชทใน DEPLOYMENT.md](DEPLOYMENT.md#chat-upgrade) ห้ามรัน `schema.sql` ทั้งไฟล์ซ้ำบนฐานข้อมูลเดิม
